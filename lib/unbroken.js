...
class Checker {
  constructor(options) {
    ...
    try {
        ...
        this.exclusions = contents.filter(x => x.startsWith('!')).map(x => path.normalize(x.slice(1)));
    } catch (e) {
        this.suppressions = [];
        this.exclusions = [];
    }
  ...
  async RecurseFindMarkdownFiles(dirPath) {
    ...
    await asyncForEach(files, async file => {
      ...
      const relFilePath = this.getRelativeFilePath(filePath);
      const shouldSkip = checker.exclusions.find(x => relFilePath.startsWith(x));
      if (shouldSkip) { 
  ...
  ValidateSection(name, value, contents, filePath) {
    ...
    if (anchors.indexOf(sectionAnchor.toLowerCase()) < 0) {
      if (anchors.indexOf(sectionAnchor.replace(/\./g, '').toLowerCase()) < 0) {
        ...
        {
          this.errors.push(`Section ${sectionAnchor} not found in ${this.normalizeSlashes(this.getRelativeFilePath(filePath))}${extra}`);
        }
      }
    }
  }
