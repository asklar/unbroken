...
class Checker {
  ...
  async ValidateURL(name, value, filePath) {
    try
    {
      var response = await request(value, {
        method: 'GET',
        headers: {}
      });
    } catch (e) {
      this.errors.push(`URL not found ${value} while parsing ${filePath} (HTTP ${e})`);
      return false;
    }
    if (response.statusCode != 200) {
      this.errors.push(`URL not found ${value} while parsing ${filePath} (HTTP ${response.statusCode} ${response.statusMessage})`);
      return false;
    } else {
      return true;
    }
