...
class Checker {
  ...
  static optionDefinitions = [
    ...
    { name: 'parse-ids', type: Boolean, description: 'Allows anchors to point to Docusaurus id aliases'},
    { name: 'user-agent', alias: 'u', type: String, description: `The User-Agent string to use. Default is ${DefaultUserAgent}` },
  ];
  ...
  async Process(dirPath) {
    ...
    await this.RecurseFindMarkdownFiles(dirPath, x => this.VerifyMarkDownFile(x));
    let n = 0;
  ...
  async ValidateURL(name, value, filePath) {
    ...
    for (let i = 0; i < maxIterations; i++) {
      ...
      try {
        ...
        var r = await axios.get(value, { headers: { 'User-Agent': userAgent } });
        if (r.status == 200) {
          return true;
        }
      }
